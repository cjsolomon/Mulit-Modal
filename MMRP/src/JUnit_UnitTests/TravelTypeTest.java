...
public class TravelTypeTest {
	...
	public void testUpdateLoad() {
		String ttest_string = new String("JUnit TravelTypeTest.testUpdateLoad");
		test_type.setServiceType(ttest_string);
		test_type.setTravelTypeMode("BIKE");

		Assert.assertTrue(test_type.isNew());
		test_type.Update();
		Assert.assertFalse(test_type.isNew());

		ArrayList<TravelType> type_list = TravelType.LoadAll("where ServiceType = '" + ttest_string + "'" );
		if (!type_list.isEmpty()) {
			Assert.assertEquals(test_type.getVehicleTypeID(), type_list.get(0).getVehicleTypeID());
			Assert.assertEquals(test_type.getExplosives(),type_list.get(0).getExplosives());
			Assert.assertEquals(test_type.getHazmat(),type_list.get(0).getHazmat());
			Assert.assertEquals(new Double(test_type.getMaxCap()), new Double(type_list.get(0).getMaxCap()));
			Assert.assertEquals(new Double(test_type.getMaxWeight()), new Double(type_list.get(0).getMaxWeight()));
			Assert.assertEquals(new Double(test_type.getMinCap()), new Double(type_list.get(0).getMinCap()));
			Assert.assertEquals(test_type.getRadiation(),type_list.get(0).getRadiation());
			Assert.assertEquals(test_type.getRefridgeration(),type_list.get(0).getRefridgeration());
			Assert.assertEquals(test_type.getServiceType(), type_list.get(0).getServiceType());
			Assert.assertEquals(test_type.getTracking(),type_list.get(0).getTracking());
			Assert.assertEquals(test_type.getTrailer1(), type_list.get(0).getTrailer1());
			Assert.assertEquals(test_type.getTrailer2(), type_list.get(0).getTrailer2());
			Assert.assertEquals(test_type.getTravelTypeMode(), type_list.get(0).getTravelTypeMode());
			Assert.assertEquals(test_type.getTravelTypeName(),type_list.get(0).getTravelTypeName());
		}
		else {
			Assert.assertTrue("List should not return empty", false);

		}

		Assert.assertFalse(test_type.isDirty());
		//Change a few fields, ensure dirty, and then test again
		test_type.setHazTrue();
		test_type.setExpTrue();
		test_type.setRadTrue();
		Assert.assertTrue(test_type.isDirty());
		test_type.Update();
		type_list = TravelType.LoadAll("where ServiceType = '" + ttest_string + "'" );
		if (!type_list.isEmpty()) {
			Assert.assertEquals(test_type.getVehicleTypeID(), type_list.get(0).getVehicleTypeID());
			Assert.assertEquals(test_type.getExplosives(),type_list.get(0).getExplosives());
			Assert.assertEquals(test_type.getHazmat(),type_list.get(0).getHazmat());
