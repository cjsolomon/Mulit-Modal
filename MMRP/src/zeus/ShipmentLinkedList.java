...
public class ShipmentLinkedList {
  protected Shipment head;
  protected Shipment tail;
  protected int numShipments; //no of pickuppoints
  protected int totalDemand;  //total no of customers for all the pickup points
   ...
   * list.
   */
  public void insertLast(Shipment shipment) {
    //if there are no students, then insert student as first location
    if (head == null) {
      tail = head = shipment;
    }
    else { //insert student at last location
      tail.setNext(shipment);
      shipment.setPrev(tail);
    }
