...
public class NodesLinkedList
  ...
  protected int truckNum; //absolutely required as used in the optimizations
  ...
  /**
   * Abstract method to be implemented by the class inheriting the NodesLinkedListClass
   * @param theShipment shipment to be inserted
   * @return boolean true if the shipment was inserted
   */
    ...
    try {
      while (tempCell != tail) { //loop through all the cells until last depot
        ...
        if (!tempCell.getPickupPointName().equals(pupName)) {
          ...
          firstCellInPUP.theShipment.tempDemand = firstCellInPUP.theShipment.
              getDemand();
